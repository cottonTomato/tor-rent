...
import {
  ...
  DollarSign, Clock, Search, CheckCircle,
  Download, X
} from "lucide-react";
...
const TenantDashboard = () => {
  ...
  const actions = [
    ...
    { label: "View Complaints", icon: AlertCircle, color: "red", url: "/tenant-complaints" },
    { label: "Make Payment", icon: CreditCard, color: "green", url: "/payment" },
    { label: "View Documents", icon: FileText, color: "purple", url: "/documents" }
  ];
...
const [propertyDetails, setPropertyDetails] = useState({
  ...
  const loadDashboardData = async () => {
    ...
    const mockPayments = [
      { id: 1, type: "Rent", amount: 2500, due: "2024-03-01", status: "upcoming" },
    ...
    const mockNotifications = [
      ...
      { id: 3, title: "New Message", desc: "Message from your landlord", type: "info" }
    ];
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      setPayments([{ id: payments.length + 1, type: "Rent", amount: parseFloat(paymentForm.amount), due: paymentForm.paymentDate, status: "processing" }, ...payments]);
      setToastMessage("Payment Initiated: Your payment is being processed.");
      setIsPaymentModalOpen(false);
    } catch (error) {
      setToastMessage("Payment Failed: There was an error processing your payment.");
    }
  };
        ...
        <Card className="bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700 mb-8">
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
              <div className="space-y-2">
                <h2 className="text-2xl font-bold text-white">{propertyDetails.address}</h2>
                <p className="text-gray-400">Lease ends on {propertyDetails.leaseEnd}</p>
              </div>
              <div className="mt-4 md:mt-0 flex space-x-4">
                <Dialog open={open} onOpenChange={setOpen}>
                  <DialogTrigger asChild>
                    <Button className="bg-blue-600 hover:bg-blue-700">
                      <MessageSquare className="w-4 h-4 mr-2" /> Complaint file to Landlord
                    </Button>
                  </DialogTrigger>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="lg:col-span-2 bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700">
            <CardHeader className="p-6">
              <div className="flex items-center justify-between">
                <CardTitle className="text-xl font-semibold text-white">Upcoming Payments</CardTitle>
            ...
            <CardContent className="p-6 pt-0">
              <div className="space-y-4">
                {payments.map((payment) => (
                  <div key={payment.id} className="flex items-center justify-between p-4 bg-black/20 rounded-lg">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-400">Due {payment.due}</p>
                      </div>
                    </div>
